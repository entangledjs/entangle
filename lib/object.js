...
DO.prototype._onchange = function(changes){
  ...
  var id = this._id;
  this._save();
  if (allPrivate(changes)) return debug('ignoring private changes');
...
/**
 * Check if all `changes` are private.
 *
 * @param {Array} changes
 * @return {Boolean}
 * @api private
...
function allPrivate(changes) {
  return changes.every(function(change){
    return isPrivateKey(change.name);
  });
}
